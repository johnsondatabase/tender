    ...
    const getApiKey = () => {
        ...
        return 'AIzaSyDMMoL4G5FDGPNUB2e84XNsNIQo68USVdQ';
    };
    ...
    export async function onShowDashboardView() {
        ...
        // Inject Dashboard Template (Header Removed)
        container.innerHTML = `
            <div class="flex flex-col h-full bg-gray-50 dark:bg-gray-900 p-4 md:p-6 overflow-y-auto custom-scrollbar">
                ...
                <div class="mb-4 flex justify-end items-center gap-2">
                    <!-- AI Button -->
                    <button id="btn-ai-analyze" class="flex px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg hover:from-purple-700 hover:to-indigo-700 transition-all shadow-md items-center gap-2 font-medium text-sm">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                        <span class="whitespace-nowrap">Trợ lý AI</span>
                    </button>
                    <!-- Refresh Button -->
                    <button id="refresh-dashboard-btn" class="p-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors shadow-sm">
            ...
            <!-- AI Chat Modal -->
            <div id="ai-insight-modal" class="hidden fixed inset-0 z-[10000] flex items-center justify-center modal-backdrop p-4 bg-black/50 backdrop-blur-sm">
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-3xl flex flex-col h-[85vh] transform transition-all scale-100 overflow-hidden">
                    <div class="p-4 border-b dark:border-gray-700 bg-gradient-to-r from-purple-600 to-indigo-600 text-white flex justify-between items-center flex-shrink-0">
                        <div class="flex items-center gap-3">
                            <div class="p-2 bg-white/20 rounded-full">
                                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                            </div>
                            <div>
                                <h3 class="text-lg font-bold">Trợ lý Phân tích Kinh doanh AI</h3>
                                <p class="text-xs text-purple-100 opacity-90">Powered by Gemini 2.5 Flash</p>
                            </div>
                        </div>
                        <button id="close-ai-modal" class="text-white/80 hover:text-white p-1 rounded-full hover:bg-white/10 transition-colors">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                        </button>
                    </div>
                    
                    <!-- Chat Messages Area -->
                    <div id="ai-chat-messages" class="flex-1 overflow-y-auto p-4 bg-gray-50 dark:bg-gray-900 custom-scrollbar space-y-4">
                    ...
                    <div class="p-4 border-t dark:border-gray-700 bg-white dark:bg-gray-800 flex-shrink-0">
                        <form id="ai-chat-form" class="relative">
                            <input type="text" id="ai-user-input" class="w-full pl-4 pr-12 py-3 rounded-full border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-purple-500 focus:outline-none" placeholder="Hỏi AI về dữ liệu (VD: Tỉ lệ Win của Nguyễn Văn A)...">
                            <button type="submit" class="absolute right-2 top-2 p-1.5 bg-purple-600 text-white rounded-full hover:bg-purple-700 transition-colors">
    ...
    function setupAIButton() {
        ...
        if (btn) {
            btn.addEventListener('click', startAIChat);
        }
    ...
    // Convert dataset to a minimized CSV format for token efficiency
    function prepareDataContext() {
        // Only include essential columns: PSR, Status, Quota, SL Trung, Ma VT
        // Aggregate by PSR/Product might be safer if too many rows, but let's try raw rows limit
        const header = "PSR,Product,Status,Quota,WinQty,Hospital";
        const rows = rawDetails.slice(0, 800).map(d => { // Limit to 800 rows to fit context
            return `${d.psr || 'Unknown'},${d.ma_vt || ''},${d.tinh_trang || 'Waiting'},${d.quota || 0},${d.sl_trung || 0},"${d.ma_thau || ''}"`;
        });
        return header + "\n" + rows.join("\n");
    }
    ...
    async function startAIChat() {
        ...
        messagesDiv.innerHTML = ''; // Clear history on open
        appendAIMessage("Đang tổng hợp dữ liệu, vui lòng đợi...", true);
        ...
        try {
            ...
            chatSession = aiClient.chats.create({
                ...
                config: {
                    systemInstruction: `
                        You are an expert Sales Analyst for WH-B4 CRM.
                        You are analyzing a dataset provided in CSV format.
                        
                        DATASET FORMAT: PSR,Product,Status,Quota,WinQty,Hospital (Code)
                        
                        YOUR TASKS:
                        1. When asked for an overview, provide a concise summary with bullet points (<ul><li>).
                        2. When asked specific questions (e.g., about a specific PSR or Product), query the provided CSV data to calculate answers.
                        3. Format outputs using clear HTML (<b>, <ul>, <li>, <br>). Do NOT use markdown code blocks.
                        4. Be professional, decisive, and insightful.
                        5. If data is missing or ambiguous, state it clearly.
            ...
            const response = await chatSession.sendMessage({ message: "Hãy phân tích tổng quan tình hình kinh doanh hiện tại. Đưa ra 3 điểm mạnh, 3 rủi ro và 3 hành động đề xuất dưới dạng gạch đầu dòng." });
            ...
            messagesDiv.innerHTML = '';
            appendAIMessage(`Lỗi kết nối AI: ${e.message}. Vui lòng thử lại sau.`);
            console.error(e);
        }
    }
    ...
    async function handleUserMessage() {
        ...
        loadingDiv.innerHTML = `<div class="bg-gray-200 dark:bg-gray-700 text-gray-500 rounded-2xl rounded-tl-none py-2 px-4 text-xs italic animate-pulse">Đang suy nghĩ...</div>`;
        ...
        try {
            ...
            document.getElementById(loadingId).remove();
            appendAIMessage("Xin lỗi, đã có lỗi xảy ra. " + e.message);
        }
    }
    ...
    function appendUserMessage(text) {
        ...
        msgDiv.innerHTML = `<div class="bg-blue-600 text-white rounded-2xl rounded-tr-none py-2 px-4 max-w-[80%] text-sm shadow-sm">${text}</div>`;
    ...
    function appendAIMessage(text, isLoading = false) {
        ...
        const contentClass = isLoading ? "italic text-gray-500" : "";
        
        msgDiv.innerHTML = `
            <div class="bg-white dark:bg-gray-700 text-gray-800 dark:text-white rounded-2xl rounded-tl-none py-3 px-4 max-w-[90%] text-sm shadow-sm border border-gray-100 dark:border-gray-600 prose dark:prose-invert ${contentClass}">
                ${text}
            </div>
        `;
        div.appendChild(msgDiv);
